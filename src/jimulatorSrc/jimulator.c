...
#define reg_current                                                       \
    0              /* Values to force register accesses to specified bank \
                    */
...
unsigned char wotrustring[] = {WOTLEN - 1, /* Length of rest of record HERE */
                               (WOTLEN - 3) & 0xFF,
                               ((WOTLEN - 3) >> 8) &
                                   0xFF, /* Length of rest of message (H) */
                               1,
                               0,
...
unsigned char
    run_until_status; /*   to finish a `stepped' subroutine, SWI, etc. */
 ...
 * @param command
 */
void monitor_options_misc(unsigned char command) {
    ...
    switch (command & 0x3F) {
        ...
        case BR_FR_WRITE: {
            ...
            while (length-- > 0) {
                ...
                if (pBuff != NULL)
                    put_buffer(pBuff, tempchar); /*  and put in buffer */
            }
            emul_sendchar(0);
        } break;
 ...
 * @param c
 */
void monitor_memory(unsigned char c) {
    int addr;
    char *pointer;
    int size;
    ...
    if ((c & 0x30) == 0x10) {
        ...
        emul_getbN(&size, 2);
        size *= 1 << (c & 7);
        if (((unsigned char *)pointer + size) > ((unsigned char *)memory + RAMSIZE))
            pointer -= RAMSIZE;
        if (c & 8)
 ...
 * @param c
 */
void monitor_breakpoints(unsigned char c) {
    runflags = c & 0x3F;
 ...
 * @return int
 */
int emul_getchar(unsigned char *to_get) { return emul_getchararray(1, to_get); }
 ...
 * @param val_ptr the place to send the bytes to.
 * @param N the number of bytes to be sent.
 * @return int Returns the number of bytes received successfully (i.e. N=>"Ok")
 */
int emul_getbN(int *val_ptr, int N) {
    char buffer[MAX_SERIAL_WORD];
    int i, No_received;
 ...
 * @param char_number the number of characters.
 * @param data_ptr the characters to be read.
 ...
 * @param char_number the number of bytes to send.
 * @param data_ptr write the data from this buffer - pointer points to the
...
/**
 * @brief Breakpoints - ignoring lots of things yet. Needs amalgamating with
 * watchpoint checker
 * @param instr_addr
 * @param instr
 * @return int
...
int check_breakpoint(unsigned int instr_addr, unsigned int instr) {
    ...
    for (int i = 0, may_break = FALSE; (i < NO_OF_BREAKPOINTS) && !may_break;
         i++) {
        /* Search breakpoints */
...
/**
 * @brief Link is performed if "link" is NON-ZERO
 * @param Rm
 * @param link
 */
void bx(unsigned int Rm, int link) {
 ...
 * @param Rn
 * @param reg_list
 * @param write_back zero/non-zero Booleans
 * @param hat zero/non-zero Booleans
 */
void ldm(int mode, int Rn, int reg_list, int write_back, int hat) {
...
/**
 * @brief This is the breakpoint *instruction* - Looks like a prefetch abort
 */
void breakpoint() {
    spsr[abt_mode] = cpsr;
...
/**
 * @brief Modified "get_reg" to give unadulterated copy of PC
 * @param reg_no
 * @param force_mode
 * @return int
 */
int get_reg_monitor(int reg_no, int force_mode) {
    if (reg_no != 15)
        return get_reg(reg_no, force_mode);
    else
...
/**
 * @brief
 * @return the length, in bytes, of the currently expected instruction. (4 for
 * ARM, 2 for Thumb)
 */
int instruction_length() {
    if ((cpsr & tf_mask) == 0)
 ...
 * @param source
 */
void write_mem(unsigned int address, int data, int size, boolean T,
               int source) {
    unsigned int mask;
...
/**
 * @brief to be completed - Needs privilege information
 * @param address
 * @param data
 * @param size
 * @param direction
...
/**
 * @brief Get the number object (this is filthy :-/ )
 * @param ptr
 * @return int
 */
int get_number(char *ptr) {
...
/**
 * @brief As the compiler can't manage it
 * @param value
 * @param distance
 * @param cf is -internal- Boolean
 * @return int
 */
int lsl(int value, int distance, int *cf) {
    int result;
            ...
            {      /* NO FLAG UPDATE */
                switch (op_code & 0X0300) {
                    ...
                    case 0X0100: /* CMP (3) high registers */
                        rd =
                            get_reg((((op_code & 0X0080) >> 4) | (op_code & 7)), reg_current);
                        rm = get_reg(((op_code >> 3) & 15), reg_current);
                        set_flags(flag_sub, rd, rm, rd - rm, 1);
                        break;
...
void sp_pc(unsigned int op_code) {
    ...
    if ((op_code & 0X1000) == 0) /* ADD SP or PC */
    {
        ...
        if ((op_code & 0X0800) == 0) /* ADD(5) -PC */
            data =
                (get_reg(15, reg_current) & 0XFFFFFFFC) + ((op_code & 0X00FF) << 2);
...
/**
 * @brief Jesus wept.   What was wrong with "read_mem" and "write_mem"? If int <
 * 32 bits the whole lot is broken anyway!
 * @param number
 * @return unsigned int
 */
unsigned int getmem32(int number) {
...
/**
 * @brief Terminal support routines
 * @param buffer
 */
