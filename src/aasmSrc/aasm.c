...
typedef enum { NO_LABEL,
               MAYBE_SYMBOL,
               SYMBOL,
               LOCAL_LABEL } label_type;
typedef enum { ALL,
               EXPORTED,
               DEFINED,
               UNDEFINED } label_category;
typedef enum { ALPHABETIC,
               VALUE,
               DEFINITION,
               FOR_ELF } label_sort;
...
void print_error(char *line, unsigned int line_no, unsigned int error_code,
    ...
    for (i = 0; line[i] != '\0'; i++) printf("%c", line[i]);
    printf("\n"); /* Yuk! */
...
unsigned int parse_source_line(char *line, sym_table_item *mnemonic_list,
    ...
    if (!test_eol(line[pos])) /* Something on line - not comment */
    {
        if (get_num(line, &pos, &value, 10)) /* Look for a `local' (numeric) label */
        {
            ...
            /* Element=>buffer */
            {
                ...
                if ((ptr = sym_find_label_list(buffer, mnemonic_list)) == NULL) { /* Not a mnemonic */
        ...
        /* Could check for other symbols (e.g. "=") first     @@@@ */
        {
            ...
            if ((j = get_identifier(line, pos, buffer, LINE_LENGTH)) != 0) { /* Possible identifier found */
    ...
    if (last_pass) {
        ...
        if ((label_this_line.sort == SYMBOL) && ((label_this_line.symbol->flags & SYM_REC_EQUATED) == 0))
    ...
    void assemble_define(int size) { /* Elongated by string definition */
        ...
        while (!terminate) {
            ...
            {                                                            /* Input string */
                ...
                /* Parse expression */
                if ((error_code == eval_okay) || allow_error(error_code, first_pass, last_pass)) {
        ...
        if (error_code == eval_okay) {
            ...
            {                            /* Explicit rotation code */
                ...
                if (x >= 0)
                    ...
                    if (translate && (trans_type[i] >= 0)) { /* Maybe can do as a different op. */
    ...
    unsigned int parse_reg_list(unsigned int allowed) {
        ...
        do {
            ...
            if ((reg = get_thumb_reg(line, &position, allowed)) >= 0) {
                ...
                if (cmp_next_non_space(line, &position, 0, '-')) { /* Register list coming up */
                    if ((reg = get_thumb_reg(line, &position, allowed)) >= 0) {
        ...
        if (cmp_next_non_space(line, &position, 0, '[')) {
            if ((reg = get_reg(line, &position)) >= 0) {
                ...
                switch (line[position]) /* What succeeds base register? */
                {
                    ...
                    case ',':
                        if (cmp_next_non_space(line, &position, 1, '#')) { /* Pre-index immediate */
                            op_code = op_code | parameter[0];
                            error_code = evaluate(line, &position, &value, symbol_table);
                            op_code = ldr_offset(op_code, value, size);
                            /* Offset */
                            if ((error_code == eval_okay) || allow_error(error_code, first_pass, last_pass)) { /* May need to tolerate an error to check syntax */
                                if (line[position] == ']') {
                                    if (cmp_next_non_space(line, &position, 1, '!'))
                            ...
                            if (size != TYPE_CPRO) {
                                ...
                                if ((reg = get_reg(line, &position)) >= 0) {
                                    ...
                                    if (!cmp_next_non_space(line, &position, 0, ']')) { /* Something else here ... */
            ...
            switch (token & 0x000F0000) {
                ...
                case 0x00010000:                   /* Data operations */
                    ...
                    if (error_code == eval_okay) {
                        ...
                        if (error_code == eval_okay) {                       /* Rm always present, so check omitted */
                    ...
                    while ((error_code == eval_okay) && (pRegs[i] >= 0)) {
                        if ((reg = get_reg(line, &position)) >= 0) {
                            if (rrx && (i == 2))
                                ...
                                } else if ((token & 0x0000F000) == 0x00002000) /* SWP addr. bodged */
                                {
                                    if (!cmp_next_non_space(line, &position, 0, ','))
                                        error_code = SYM_NO_COMMA | position;
                ...
                case 0x000C0000: /* LDRD/STRD */
                {
                    ...
                    switch (token & 0x000F0000) {
                        ...
                        case 0x00030000:                                                    /* LDR/STR only */
                            if ((error_code == eval_okay) && ((token & 0x00001000) != 0)) { /* 'T' option */
                ...
                case 0x00050000: /* Miscellany */
                    switch (token & 0x0000F000) {
                                ...
                                if ((token & 0xF0000000) == 0xE0000000) /* Only unconditional */
                            {
                                ...
                                if ((value & 1) != 0) /* Not halfword aligned */
                                    error_code = SYM_UNALIGNED_BRANCH;
                                         ...
                                         || ((value & 0xFE000000) == 0xFE000000))
                                    op_code = 0xFA000000 | ((value & 0x03FFFFFC) >> 2) | ((value & 2) << 23);
                                else
                                    error_code = SYM_OORANGE_BRANCH;
                            } else
                        ...
                        case 0x00004000: /* PLD */
                            ...
                            if ((op_code & 0x01200000) != 0x01000000) { /* Modes which write back are disallowed */
                                op_code = 0x00000000;
                                error_code = SYM_ADDR_MODE_BAD;
                            }
                            break;
                ...
                case 0x000A0000: /* CDP + MCR/MRC */
                {
                    ...
                    if ((reg = get_copro(line, &position)) >= 0) /* `reg' is a temp var. */
                    {
                        ...
                        if (cmp_next_non_space(line, &position, 0, ',')) {
                            error_code = evaluate(line, &position, &value, symbol_table);
                            if ((error_code == eval_okay) || (first_pass && ((error_code & ALLOW_ON_FIRST_PASS) != 0))) {
...
#define TOKEN_BICX 0x03C00000
                ...
                case 0x000D0000: /* Long data-op. variants */
                {
                    ...
                    else /* Only derive op. code(s) on last pass */
                    {
                        ...
                        for (i = 0; i < true_count; i++) {               /* After first word accumulate within Rd */
                    ...
                    if (reg >= 15)
                        ...
                        } else /* Fixed offset form */
                        {
                            ...
                            if (error_code == eval_okay) {
                                ...
                                if ((value & 1) != 0)         /* Not halfword aligned */
                                    error_code = SYM_UNALIGNED_BRANCH;
                                else if (((value & 0xFFC00000) == 0x00000000) || ((value & 0xFFC00000) == 0xFFC00000)) {
                                    if (last_pass) {
                                    ...
                                    if (reg < 0)
                                        ...
                                        switch (reg) /* Select according to base register */
                                        {
                                            ...
                                            case 15: /* Other legal Thumb base registers */
                                                if (imm && (((reg == 15) && ((token & 0x00002000) != 0)) || ((reg == 13) && ((token & 0x00001000) != 0)))) {
        ...
        if ((token & 0xF0000000) == 0xF0000000) { /* Directive */
            switch (token) {                      /* Defining code */
                ...
                {                // Allow " " around name?  @@@
                    ...
                    if (test_eol(line[position])) /* Effective EOL? */
                    {
                        ...
                        *include_name = (char *)malloc(LINE_LENGTH + 1); /* Overkill-so what? */
                        i = 0;
                        while (!test_eol(line[position]) && (line[position] != ' ') && (line[position] != '\t'))
                ...
                case 0xF8040000: /* RECORD */
                    error_code = evaluate(line, &position, &temp, symbol_table);
                    if (((error_code & 0xFFFFFF00) == eval_no_operand) || allow_error(error_code, first_pass, last_pass)) {
    ...
    if (first_pass && (error_code == eval_okay)) { /* Check that nothing remains on line (first pass only) */
        position = skip_spc(line, position);
...
int get_thing(char *line, unsigned int *pos, sym_table *table) {
    ...
    *pos = skip_spc(line, *pos);
    ...
    if ((i = get_identifier(line, *pos, buffer, THING_BUFFER_LENGTH)) > 0) { /* Something taken */
...
int get_creg(char *line, unsigned int *pos) { return get_thing(line, pos, cregister_table); }

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

int get_copro(char *line, unsigned int *pos) { return get_thing(line, pos, copro_table); }
...
int get_psr(char *line, unsigned int *pPos) {
    ...
    if (reg >= 0) {
        if (((line[(*pPos) + 1] & 0xDF) == 'P') && ((line[(*pPos) + 2] & 0xDF) == 'S') && ((line[(*pPos) + 3] & 0xDF) == 'R')) {
            *pPos = *pPos + 4;
            if (line[(*pPos)] != '_')
...
int data_op_imm(unsigned int value) /* Not particularly efficient */
{
    ...
    unsigned int rol32(unsigned int x, unsigned int j) { return (((x & 0xFFFFFFFF) >> (32 - j)) | (x << j)) & 0xFFFFFFFF; }
...
void assemble_redef_label(unsigned int value, int defined, own_label *my_label,
    ...
    {
        if ((my_label->sort == SYMBOL) || (my_label->sort == MAYBE_SYMBOL)) { /* Symbolic (ordinary) label on current line */
            old_value = my_label->symbol->value;
            flags = my_label->symbol->flags;
        } else if (my_label->sort == LOCAL_LABEL) { /* Local label on current line */
...
void literal_dump(boolean last_pass, char *line, unsigned int limit) {
    ...
    while ((literal_tail != literal_head) && ((limit == 0) || ((address + (((literal_tail->flags & LIT_HALF) != 0) ? 2 : 4)) <= limit))) { /* Something to dump and space to dump it in */
        ...
        if ((literal_tail->flags & LIT_NO_DUMP) == 0) /* If -not- converted to MOV */
        {
            ...
            if ((fList != NULL) && ((i != 0)                                     /* Needed to align first */
...
void elf_dump(unsigned int address, char value) {
    ...
    if (elf_new_block || (elf_section != elf_section_old)) { /* New or unexpected address */
...
void elf_dump_out(FILE *fElf, sym_table *table) {
    ...
    elf_dump_word(fElf, prog_start + total + symtab_length + strtab_length + shstrtab_length);
    elf_dump_word(fElf, prog_start + total + symtab_length + strtab_length + shstrtab_length + (ELF_PHENTSIZE * fragments));
...
int sym_delete_record_list(sym_record **ptr1, int export) {
    ...
    while (*ptr1 != NULL) /* While not end of list ... */
    {
        ...
        if (!export || ((ptr2->flags & SYM_REC_EXPORT_FLAG) == 0)) { /* Delete record */
...
void sym_delete_record(sym_record *old_record) { /* Can deallocate strings etc. if such have been allocated @@ */
    free(old_record);
    return;
}
                ...
                while ((ptr2 != NULL) && !found) {
                    switch (how) /* Field used for sorting */
                    {
                        ...
                        case VALUE: /* Sort numerically */
                            after = (temp_record->value > ptr2->value) && ((temp_record->flags & SYM_REC_DEF_FLAG) != 0);
                            break;
                        ...
                        case FOR_ELF: /* In order of definition, locals first */
                            after = ((((temp_record->flags & SYM_REC_EXPORT_FLAG) != 0) && ((ptr2->flags & SYM_REC_EXPORT_FLAG) == 0)) || (temp_record->identifier > ptr2->identifier));
                            break;
                    }
...
unsigned int evaluate(char *string, unsigned int *pos, int *value,
    ...
    void Eval_inner(int priority, int *value) { /* Main function shares stack etc. */
        ...
        while (!done) {
            ...
            if (error == eval_okay) {
                ...
                if (error == eval_okay) {
                    ...
                    if ((error = get_operator(string, pos, &operator, & priority)) == eval_okay) {
                        while ((priority <= math_stack[math_SP - 1]) && (math_stack[math_SP - 1] > 1)) { /* If priority decreasing and previous a real operator, OPERATE */
            ...
            if (error != eval_okay) {
                ...
                else if (error != eval_div_by_zero) /* Arithmetic error will occur late */
    ...
    *value = 0;
    ...
    if (input[ii] == '(') { /* Open brackets instead of value */
        *bracket = TRUE;
        ...
        if ((i = get_identifier(input, ii, ident, LINE_LENGTH)) > 0) { /* Something taken */
            if ((symbol = sym_find_label(ident, symbol_table)) != NULL) {
                if ((symbol->flags & SYM_REC_DEF_FLAG) != 0) { /* Label present and with a valid value */
                    *value = symbol->value;
                    status = eval_okay;
            ...
            if (input[ii] == '\%') {
                ...
                if ((evaluate_own_label->sort != LOCAL_LABEL) && ((directions & 1) == 0)) { /* If searching forwards only and no local label on this line */
                    if (loc_lab_position == NULL)
    ...
    *pos = skip_spc(input, *pos); /* In case of error want this at next item */
    ...
    switch (input[ii]) {
        ...
        default: { /* Have a go at symbolically defined operators */
            ...
            if ((i = get_identifier(input, ii, buffer, SYM_NAME_MAX)) > 0) {  /* Something taken */
                if ((ptr = sym_find_label(buffer, operator_table)) != NULL) { /* Symbol recognised */
                    *operator= ptr->value;
                    ii += i;
                    status = eval_okay;
                }
...
boolean test_eol(char character) { return (character == '\0') || (character == ';') || (character == '\n'); }
...
int get_num(char *line, int *position, int *value, unsigned int radix) {
    int num_char(char *line, int *pos, unsigned int radix) { /* Return value(c) if in radix  else return -1 */
        char c;
...
int allow_error(unsigned int error_code, boolean first_pass, boolean last_pass) {
    return (!last_pass && ((error_code & ALLOW_ON_INTER_PASS) != 0)) || (first_pass && ((error_code & ALLOW_ON_FIRST_PASS) != 0));
}
